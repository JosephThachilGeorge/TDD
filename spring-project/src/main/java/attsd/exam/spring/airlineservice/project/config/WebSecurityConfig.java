package attsd.exam.spring.airlineservice.project.config;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers(PUBLIC).permitAll().antMatchers(PRIVATE).authenticated().and().csrf()
				.disable().formLogin().successHandler(new AirlineserviceAuthenticationSuccessHandler()).loginPage(LOGIN)
	...
	private class AirlineserviceAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
