package attsd.exam.spring.loginservice.project.config;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers(PUBLIC).permitAll().antMatchers(PRIVATE).authenticated().and().csrf()
				.disable().formLogin().successHandler(new LoginserviceAuthenticationSuccessHandler()).loginPage(LOGIN)
	...
	private class LoginserviceAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
